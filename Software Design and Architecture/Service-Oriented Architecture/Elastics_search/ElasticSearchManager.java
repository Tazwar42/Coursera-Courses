...
public class ElasticSearchManager {
    ...
    private static final String INDEX = "725265417"; // TODO: MUST CHANGE THIS to random number string
    ...
    public static class AddBidTask extends AsyncTask<Bid,Void,Boolean> {
        ...
        protected Boolean doInBackground(Bid... params) {
            ...
            Index index = new Index.Builder(bid).index(INDEX).type(ITEM_TYPE).id(id).build();
            try {
                DocumentResult execute = client.execute(index);
                if(execute.isSucceeded()) {
                    ...
                    Log.e("ELASTICSEARCH", "Add nid failed");
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    /**
     * Delete id from remote server using bid_id
     */
